...
/// Class that represents tf node that is being returned by tf2_frames service
...
/// Class that represents internal tree node with children
#[derive(Debug)]
struct TreeNode {
    name: String,
...
impl TransformationDetails {
    fn from_tf(tf_stamped: rosrust_msg::geometry_msgs::TransformStamped) -> TransformationDetails {
        ...
        TransformationDetails {
            from_frame: tf_stamped.header.frame_id.clone(),
            to_frame: tf_stamped.child_frame_id.clone(),
    ...
    fn to_spans(&self) -> Vec<Spans<'static>> {
        vec![
            Spans::from(Span::styled(
                format!("From {}", self.from_frame),
                Style::default().add_modifier(Modifier::ITALIC),
            )),
            Spans::from(Span::styled(
                format!("To {}", self.to_frame),
                Style::default().add_modifier(Modifier::ITALIC),
            )),
            Spans::from(Span::styled(
                "Translation",
...
impl<'a> TfTreeView<'a> {
    ...
    fn echo_selected_and_picked(&mut self) {
        ...
        self.tf_echo_details = match self.viewport.borrow().tf_listener.lookup_transform(
            ...
            Err(_) => vec![
                Spans::from(Span::raw(format!("From {}", &picked_item))),
                Spans::from(Span::raw(format!("To {}", &selected_item))),
                Spans::from(Span::styled(
...
impl<'a> AppMode for TfTreeView<'a> {
    fn run(&mut self) {
        ...
        if let Some(node) = self.nodes.get(string_representation.as_str()) {
            self.status_bar = format!(
                "rate: {:e}, most_recent_tf: {}, oldest_transform: {}",
